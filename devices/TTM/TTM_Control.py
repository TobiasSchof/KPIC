...
NPS_PORT = 7 #The NPS port that this device is on
...
class __TTM:
    """Class to control the tip tilt mirror. NOT FOR USE BY GENERAL SCRIPTS

    Note: this class interacts directly with the TTM. If you want to use the 
    TTM, please use the shared memory structure or the higher level library.
    """

    def __init__(self):
        """Constructor for TTM control code."""

        #cleanup handles keyboard interupts and exceptions
    ...
    def loop(self):
        """A wrapper to continuously run an asyncio loop with a process
        to update the state shared memory and one to listen for the command
        shared memory"""
